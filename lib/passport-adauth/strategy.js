...
/**
 * Passport wrapper for adauth
 */
var passport = require('passport-strategy'),
    ADAuth   = require('adauth'),
    util     = require('util');

/**
 * Strategy constructor
 *
 * The AD authentication strategy authenticates requests based on the
 ...
 *           tlsOptions: {
 *             { ca: [ fs.readFileSync('./domain-ca.cer') ] }
 *           }
 *         }
 *       },
 *       function(user, done) {
...
var Strategy = function(options, verify) {
  ...
  if (!options) {
    throw new Error('AD authentication strategy requires options');
  }
 ...
 *                          NoSuchObjectError, and /no such user/ LDAP errors
 *                          (default: 'Invalid username/password')
 *
 * - `userNotFound`         flash message when AD returns no error but also no
 *                          user
 *                          (default: 'Invalid username/password')
 *
 * - `constraintViolation`  flash message when user account is locked
 *                          (default: 'Exceeded password retry limit, account
 *                          locked')
 */
var handleAuthentication = function(req, options) {
  var username, password, ad;
  options || (options = {});
  ...
  ad = new ADAuth(this.options.server);
  ad.authenticate(username, password, function(err, user) {
    ad.close(function(){}); // We don't care about the closing
    if (err) {
